...
for business in ['Travel', 'Retail', 'Restaurants']:
    ...
    plt.title(
        f'Line Chart - {business} - Activity Percentages for India and United States')
    plt.grid()
    plt.legend()

    # Save the plot with a filename based on country and activity percentages
    plt.savefig(f'Line Chart - {business} - Activity Percentages India and United States.png', bbox_inches='tight')
    plt.close()
...
for year in min_max_years:
    ...
    for business in ['Travel', 'Retail', 'Restaurants']:
        ...
        plt.title(
            f'Line Chart - {business} - Activity Quantiles For The Year {year}')
        plt.grid()
        plt.legend()

        # Save the plot with a filename based on country and activity quantiles
        plt.savefig(f'Line Chart - {business} - Activity Quantiles For The Year {year}.png', bbox_inches='tight')
        plt.close()
